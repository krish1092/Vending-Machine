...
function insertCoin(insertedCoin) {
    ...
    if (validated == true) {
        ...
        $.get('Vending/GetCoinValue', { CoinAsJSONString : insertedCoin })
...
function finishVending(productCount, productName) {
    if (productCount > 0) {
        ...
        if (remainingBalance >= 0) {
            ...
            //Ajax Call
            $.post('Vending/CompleteVending', { FinishVendingString: JSON.stringify({ Coins: coinsAdded, RemainingAmount: remainingBalance, ProductName: productName }) })
                         .done(function (response) {
...
function productAvailabilty(productName) {
    ...
    $.get('Vending/GetProductCount', { ProductName: productName })
        .done(function(response){
...
function refillVendingMachine() {
    $.get('Vending/RefillVendingMachine')
                     .done(function (response) {
